...
// fork runc process and prepare container
// before start with "execRunContainer"
func execCreateContainer(i *execInfo) error {
	runcDir := filepath.Join(os.Getenv("STATE_PATH_MYRUNC"), i.id) // create root dir var temp files
	if err := os.MkdirAll(runcDir, 0755); err != nil{
		return fmt.Errorf("Failed to create state dir: %v\n", err)
	}

	pipePath := filepath.Join(runcDir, os.Getenv("SIGNAL_PIPE"))
	pipePath2 := filepath.Join(runcDir, os.Getenv("GO_PIPE"))
	...
	cmd := exec.Command(os.Getenv("SELF_EXEC_PATH"),"init", i.id,"--bundle", i.bundlePath)
...
// create state.json in /var/run/myrunc/contrainer_id directory
...
type Status string
// const variables show conditions of containers
const (
	StatusCreating Status = "deleted"
...
// describe condition of runc process
type State struct {
	ID 			string `json:"id"`
	PID         int    `json:"pid"`
